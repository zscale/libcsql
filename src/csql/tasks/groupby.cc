...
//        dst_group.emplace_back(VM::allocInstance(txn_, e.program(), scratch));
//      }
//    }
//
//    for (size_t i = 0; i < select_exprs_.size(); ++i) {
...
//  if (group.size() == 0) {
//    for (const auto& e : select_exprs_) {
//      group.emplace_back(VM::allocInstance(txn_, e.program(), scratch));
//    }
//  }
//
//  for (size_t i = 0; i < select_exprs_.size(); ++i) {
...
//
//    for (size_t i = 0; i < select_exprs_.size(); ++i) {
//      VM::saveState(txn_, select_exprs_[i].program(), &group.second[i], os);
//    }
//  }
//}
//
...
//      group.emplace_back(VM::allocInstance(txn_, e.program(), scratch));
//      VM::loadState(txn_, e.program(), &group[i], is);
//    }
//  }
//
//  return true;
...
//    GroupByExpression(txn, column_names, std::move(select_expressions)),
//    params_(params),
